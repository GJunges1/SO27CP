...
void RetiraJavali(int i){
    ...
    int r = mq_receive(fd, (void*)ptr_m, sizeof(messbuf_t), 0);
    if(r < 0){
        sem_post(&ptr->cald_vazio);
        printf("Gaules %c(%d) acordou cozinheiro\n", nome[i], i);
        sem_wait(&ptr->cald_cheio);

        r = mq_receive(fd, (void*)ptr_m, sizeof(messbuf_t), 0);
        if(r < 0)
...
int main(void){
    ...
    fd = mq_open("/myqueue", O_RDONLY|O_NONBLOCK);
    if(fd < 0)
        perror("Erro ao abrir em C");
